    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4 gap-3">
                ...
                @if($restaurant)
                    ...
                    <a href="https://{{ $restaurant->subdomain }}.{{ config('app.frontend_url_base') }}" target="_blank"
                       class="text-decoration-none ms-2 link-offset-2 link-underline-opacity-0 link-underline-opacity-100-hover">
                        <i class="bi bi-link-45deg"></i> {{ $restaurant->subdomain }}
                        .{{ config('app.frontend_url_base') }}
                    </a>
                @else
        ...
        <div>
            <a href="{{ route('orders.index') }}" wire:navigate
               class="btn btn-brand rounded-pill px-4 py-2 shadow-sm w-100 w-md-auto">
        ...
        <div class="alert alert-warning border-0 shadow-sm rounded-4 p-4" role="alert">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <p class="mb-0">Anda belum mengatur informasi restoran anda. Silahkan atur sekarang untuk mulai
                        menerima pesanan.</p>
                </div>
            </div>
            <hr>
            <div class="text-end">
                <a href="{{ route('settings.index') }}" class="btn btn-dark rounded-pill px-4">Atur Restoran
                    Sekarang</a>
            </div>
        </div>
    @else
        ...
        <div class="row row-cols-1 row-cols-md-3 g-3 g-md-4 mb-5">
            ...
            <div class="col">
                <div
                    class="card h-100 border-0 shadow-sm rounded-4 position-relative overflow-hidden hover-scale transition-all">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div
                                class="bg-brand bg-opacity-10 text-brand rounded-circle p-3 d-flex align-items-center justify-content-center"
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div
                        class="card-header border-0 pt-4 px-4 pb-2 d-flex justify-content-between align-items-center bg-transparent">
                        <h6 class="fw-bold text-dark mb-0">Pesanan Terbaru</h6>
                        <a href="{{ route('orders.index') }}"
                           class="btn btn-sm btn-light rounded-pill px-3 text-muted border transition-all hover-bg-light">Lihat
                            Semua</a>
                    </div>
                    <div class="card-body p-0">
                        <div class="list-group list-group-flush">
                            @forelse($recentOrders as $order)
                                <div class="list-group-item p-3 border-0 border-bottom hover-bg-light transition-all">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <div class="d-flex align-items-center gap-2">
                                            <span
                                                class="badge bg-light text-dark border rounded-pill">#{{ $order->id }}</span>
                                            <h6 class="fw-bold text-dark mb-0 text-truncate"
                                                style="max-width: 150px;">{{ $order->customer_name }}</h6>
                                        </div>
                                        <span
                                            class="fw-bold text-brand">Rp {{ number_format($order->total_price, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="text-muted small d-flex align-items-center gap-2">
                                            <span><i class="bi bi-shop me-1"></i> {{ $order->order_type }}</span>
                                            <span class="opacity-25">|</span>
                                            <span><i class="bi bi-clock me-1"></i> {{ $order->created_at->diffForHumans() }}</span>
                                        </div>
                                        <div>
                                            @if($order->status == 'pending')
                                                <span
                                                    class="badge bg-warning text-dark rounded-pill px-3">Menunggu</span>
                                            @elseif($order->status == 'processing')
                                                <span class="badge bg-info text-white rounded-pill px-3">Diproses</span>
                                            @elseif($order->status == 'completed')
                                                <span
                                                    class="badge bg-success text-white rounded-pill px-3">Selesai</span>
                                            @else
                                                <span
                                                    class="badge bg-secondary text-white rounded-pill px-3">{{ $order->status }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-body p-4 d-flex align-items-center justify-content-between">
                            ...
                            <a href="{{ route('menu.index') }}"
                               class="btn btn-outline-dark rounded-circle p-3 transition-all hover-scale">
